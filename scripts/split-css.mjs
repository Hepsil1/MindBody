...
const MAPPINGS = {
    ...
    "checkout.css": [".checkout", ".mb-checkout", ".checkout-form", ".checkout-sidebar"],
    "contacts.css": [".contact-", ".contacts-"],
    "cart.css": [".cart-drawer"],
    "wishlist.css": [".wishlist-page", ".wishlist-hero", ".wishlist-empty"],
};
...
function matchesPrefixes(rule, prefixes) {
    ...
    return rule.selectors.some((selector) => prefixes.some((prefix) => selector.includes(prefix)));
}
                        ...
                        if (prefixes.includes(".mb-")) {
                            if (
                                !rule.selectors.some((s) =>
                                    /\.(mb-shop|mb-checkbox|mb-size|mb-color|mb-reset|mb-toolbar|mb-checkout)/.test(
                                        s,
                                    ),
                                )
                            ) {
                    ...
                    if (rule.parent && rule.parent.type === "atrule") {
                        ...
                        let existingAtRule = extractedCSS[targetFile].nodes.find(
                            (n) =>
                                n.type === "atrule" &&
                                n.name === atRuleName &&
                                n.params === atRuleParams,
                        );

                        if (!existingAtRule) {
                            existingAtRule = postcss.atRule({
                                name: atRuleName,
                                params: atRuleParams,
                            });
